...
export function addRezept(input: Omit<Rezept, "id" | "version">): Rezept {
  const neu: Rezept = { ...input, id: `r-${String(alle.length + 1).padStart(3, "0")}`, version: 1 };
  alle = [neu, ...alle];
  emit();
  return neu;
}
...
export function updateRezept(id: string, updates: Omit<Rezept, "id" | "version">) {
  alle = alle.map((r) => (r.id === id ? { ...updates, id, version: r.version + 1 } : r));
  emit();
}

export function duplicateRezept(id: string): Rezept | undefined {
  const original = alle.find((r) => r.id === id);
  if (!original) return undefined;
  const heute = new Date().toISOString().slice(0, 10);
  const kopie: Rezept = {
    ...original,
    id: `r-${String(alle.length + 1).padStart(3, "0")}`,
