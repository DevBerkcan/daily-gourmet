...
export function OrdersHistory() {
  ...
  const { pageItems, page, setPage, pageSize, setPageSize, totalPages, totalItems, pageSizeOptions } = usePagination(eigene);
  ...
  return (
    <div className="flex flex-col gap-6">
      {pageItems.map((bestellung) => {
        ...
        return (
          <Card key={bestellung.id}>
            <CardHeader
              title={`Bestellung KW ${plan?.kalenderwoche ?? "—"}`}
              hint={`Frist: ${bestellung.frist}${bestellung.abgesendetAm ? ` · Abgesendet: ${bestellung.abgesendetAm}` : ""}`}
              actions={
            ...
            />
            <div className="flex items-center justify-between gap-3 border-b border-line bg-paper/50 px-5 py-3 text-sm">
              <span className="text-muted">Bestellte Portionen gesamt</span>
              <strong className="font-display text-xl text-basil">{gesamt}</strong>
            </div>
            <Table head={["Tag", "Gericht", "Portionen", "Hinweis"]}>
              {bestellung.positionen.map((position, index) => (
                <tr key={`${position.datum}-${position.rezeptId}-${index}`}>
                  ...
                  <Td className="font-medium text-ink">{rezepte.find((r) => r.id === position.rezeptId)?.name}</Td>
                  <Td className="font-semibold">{position.portionen}</Td>
                  <Td className="text-muted">{position.hinweis ?? "—"}</Td>
                </tr>
              ))}
            </Table>
            {bestellung.status === "LOCKED" ? (
              <p className="border-t border-line px-5 py-3 text-xs text-muted">Die Frist ist abgelaufen. Korrekturen sind nur noch durch Daily Gourmet möglich und werden protokolliert.</p>
            ) : null}
          </Card>
        );
      })}
