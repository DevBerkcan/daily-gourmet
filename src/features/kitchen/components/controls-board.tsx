...
import { useKontrollen, useCreateKontrolle, KONTROLL_ARTEN, type KontrollArt } from "@/lib/services/production";
...
export function ControlsBoard() {
  const kontrollen = useKontrollen();
  const createKontrolle = useCreateKontrolle();
  const [offen, setOffen] = useState(false);
  const [art, setArt] = useState<KontrollArt>("Kerntemperatur");
  ...
  function speichern(event: FormEvent<HTMLFormElement>) {
    ...
    if (!ist.trim() || !bereich.trim()) return;
    createKontrolle.mutate(
      { art, bereich: bereich.trim(), soll: soll.trim(), ist: ist.trim() },
      { onSuccess: () => { setIst(""); setOffen(false); } }
    );
  }
    ...
    <>
      <div className="mb-6 rounded-card border border-ok/30 bg-ok-soft px-5 py-4">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div className="flex items-start gap-3"><CheckCircle2 size={21} className="mt-0.5 shrink-0 text-ok" aria-hidden /><div><p className="font-semibold text-ink">Pflichtkontrollen</p><p className="mt-1 text-sm text-muted">Erfassen Sie Temperaturen und Wareneingänge direkt am Arbeitsschritt.</p></div></div>
      ...
      {offen ? (
        <Card className="mb-6">
          ...
          <form onSubmit={speichern} className="grid gap-4 p-5 md:grid-cols-2 xl:grid-cols-4">
            <label className="flex flex-col gap-1.5 text-xs font-medium text-muted">Kontrollart<select value={art} onChange={(event) => setArt(event.target.value as KontrollArt)} className={fieldClass}>{KONTROLL_ARTEN.map((a) => <option key={a}>{a}</option>)}</select></label>
            <label className="flex flex-col gap-1.5 text-xs font-medium text-muted">Gericht / Bereich<input value={bereich} onChange={(event) => setBereich(event.target.value)} required className={fieldClass} /></label>
      ...
      <Card>
        <CardHeader title="Kontrollprotokoll" hint={`${kontrollen.length} Messungen dokumentiert`} actions={<ClipboardCheck size={19} className="text-basil" aria-hidden />} />
        <Table head={["Zeit", "Kontrolle", "Gericht / Bereich", "Sollwert", "Messwert", "Erfasst von", "Status"]}>
          {kontrollen.map((kontrolle) => <tr key={kontrolle.id}><Td className="font-semibold text-ink">{kontrolle.zeit}</Td><Td>{kontrolle.art}</Td><Td>{kontrolle.bereich}</Td><Td className="text-muted">{kontrolle.soll}</Td><Td className="font-display text-lg font-semibold text-ink">{kontrolle.ist}</Td><Td>{kontrolle.person}</Td><Td>{kontrolle.status === "OK" ? <span className="inline-flex items-center gap-1.5 text-xs font-medium text-ok"><CheckCircle2 size={15} aria-hidden />In Ordnung</span> : <span className="text-xs font-medium text-danger">Abweichung</span>}</Td></tr>)}
        </Table>
      </Card>
    </>
  );
