...
import { einrichtungen, einrichtungById } from "@/lib/data";
import { rezeptById } from "@/features/recipes/data";
import { addLieferRoute, useLieferRouten } from "../store";
import { behaelterPositionenJeRoute, fahrer, fahrerById, portionenJeRoute } from "../data";
...
export function RouteManager() {
  ...
  const [details, setDetails] = useState<string | null>(routen[0]?.id ?? null);
  const [name, setName] = useState("");
  const [datum, setDatum] = useState("2026-08-07");
  const [fahrerId, setFahrerId] = useState(fahrer[0]?.id ?? "");
  ...
  function speichern(event: FormEvent<HTMLFormElement>) {
    ...
    addLieferRoute({ name: name.trim(), datum, fahrerId, start, einrichtungIds });
    setName(""); setEinrichtungIds([]); setFormularOffen(false);
  }
      ...
      {formularOffen ? (
        <Card className="mb-6">
          ...
          <form onSubmit={speichern} className="grid gap-5 p-5 md:grid-cols-2">
            ...
            <label className="flex flex-col gap-1.5 text-xs font-medium text-muted">Fahrer<select value={fahrerId} onChange={(event) => setFahrerId(event.target.value)} className={fieldClass}>{fahrer.map((person) => <option key={person.id} value={person.id}>{person.name} · {person.kennzeichen}</option>)}</select></label>
        ...
        {routen.map((route) => {
          ...
          return <Card key={route.id}>
            <div className="flex flex-wrap items-start justify-between gap-4 px-5 py-4">
              <div><div className="flex flex-wrap items-center gap-2"><span className="rounded-full bg-info-soft px-2.5 py-0.5 text-xs font-medium text-info">{route.status === "BELADUNG" ? "In Beladung" : "Geplant"}</span><span className="text-xs text-muted">{new Date(`${route.datum}T12:00:00`).toLocaleDateString("de-DE", { weekday: "long", day: "2-digit", month: "long" })}</span></div><h2 className="mt-2 font-display text-xl font-semibold text-ink">{route.name}</h2><div className="mt-2 flex flex-wrap gap-x-5 gap-y-1 text-sm text-muted"><span className="inline-flex items-center gap-1.5"><UserRound size={15} aria-hidden />{person?.name}</span><span className="inline-flex items-center gap-1.5"><Truck size={15} aria-hidden />{person?.fahrzeug} · {person?.kennzeichen}</span><span className="inline-flex items-center gap-1.5"><Clock3 size={15} aria-hidden />{route.start}–{route.rueckkehr} Uhr</span></div></div>
            ...
            {istOffen ? <div className="border-t border-line bg-paper/50 px-5 py-5"><div className="relative ml-3 border-l-2 border-basil-soft pl-6">{route.stopps.map((stopp) => { const einrichtung = einrichtungById(stopp.einrichtungId); return <div key={stopp.id} className="relative pb-6 last:pb-0"><span className="absolute -left-[33px] flex size-4 items-center justify-center rounded-full bg-basil text-[9px] font-bold text-white">{stopp.reihenfolge}</span><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="font-semibold text-ink">{einrichtung?.name}</p><p className="mt-1 inline-flex items-center gap-1.5 text-xs text-muted"><MapPin size={13} aria-hidden />{einrichtung?.anschrift}</p><p className="mt-1 text-xs text-muted">Ankunft {stopp.ankunft} · Zeitfenster {stopp.zeitfenster}</p></div><div className="text-right">{stopp.positionen.map((position) => <p key={position.id} className="text-sm"><strong className="text-basil">{position.portionen}</strong> {rezeptById(position.rezeptId)?.name} · {position.behaelter}</p>)}</div></div></div>; })}</div><p className="mt-5 flex items-center gap-2 rounded-lg bg-surface px-3 py-2 text-xs text-muted"><CalendarDays size={14} aria-hidden />{behaelterPositionenJeRoute(route)} Ladepositionen werden automatisch an Küche und Fahrer übergeben.</p></div> : null}
          </Card>;
        })}
      </div>
    </>
