...
export function RouteManager() {
  ...
  function speichern(event: FormEvent<HTMLFormElement>) {
    ...
    const gewaehlterFahrer = fahrerId || fahrer[0]?.id;
    if (!name.trim() || einrichtungIds.length === 0 || !gewaehlterFahrer) return;
    createRoute.mutate(
      { name: name.trim(), datum, fahrerId: gewaehlterFahrer, standortId: standorte[0]?.id, start, einrichtungIds },
      ...
      {formularOffen ? (
        <Card className="mb-6">
          ...
          <form onSubmit={speichern} className="grid gap-5 p-5 md:grid-cols-2">
            ...
            <label className="flex flex-col gap-1.5 text-xs font-medium text-muted">Fahrer<select value={fahrerId || fahrer[0]?.id || ""} onChange={(event) => setFahrerId(event.target.value)} className={fieldClass}>{fahrer.map((person) => <option key={person.id} value={person.id}>{person.name} · {person.kennzeichen}</option>)}</select></label>
        ...
        {routen.map((route) => {
          ...
          return <Card key={route.id}>
            <div className="flex flex-wrap items-start justify-between gap-4 px-5 py-4">
              <div><div className="flex flex-wrap items-center gap-2"><span className="rounded-full bg-info-soft px-2.5 py-0.5 text-xs font-medium text-info">{route.status === "BELADUNG" ? "In Beladung" : "Geplant"}</span><span className="text-xs text-muted">{new Date(`${route.datum}T12:00:00`).toLocaleDateString("de-DE", { weekday: "long", day: "2-digit", month: "long" })}</span></div><h2 className="mt-2 font-display text-xl font-semibold text-ink">{route.name}</h2><div className="mt-2 flex flex-wrap gap-x-5 gap-y-1 text-sm text-muted"><span className="inline-flex items-center gap-1.5"><UserRound size={15} aria-hidden />{route.fahrerName}</span><span className="inline-flex items-center gap-1.5"><Truck size={15} aria-hidden />{person?.fahrzeug} · {person?.kennzeichen}</span><span className="inline-flex items-center gap-1.5"><Clock3 size={15} aria-hidden />{route.start}{route.rueckkehr ? `–${route.rueckkehr}` : ""} Uhr</span></div></div>
            ...
            })}</div><p className="mt-5 flex items-center gap-2 rounded-lg bg-surface px-3 py-2 text-xs text-muted"><CalendarDays size={14} aria-hidden />{behaelterPositionenJeRoute(route)} Ladepositionen werden automatisch an Küche und Fahrer übergeben.</p></div> : null}
          </Card>;
        })}
      </div>
    </>
