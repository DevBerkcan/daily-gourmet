        ...
        {route.stopps.map((stopp) => {
          ...
          return <Card key={stopp.id} className={status === "ZUGESTELLT" ? "border-ok/40" : status === "PROBLEM" ? "border-danger/40" : ""}>
            ...
            <div className="grid gap-6 p-5 lg:grid-cols-[1fr_280px]">
              ...
              <div className="flex flex-col gap-3"><a href={mapsUrl} target="_blank" rel="noreferrer" className="inline-flex min-h-11 items-center justify-center gap-2 rounded-lg bg-basil px-4 text-sm font-medium text-white hover:bg-basil-deep"><Navigation size={17} aria-hidden /> Navigation starten</a><a href={`tel:${stopp.telefon.replace(/\s/g, "")}`} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-lg border border-line-strong px-4 text-sm font-medium hover:bg-paper"><Phone size={17} aria-hidden /> {stopp.kontakt} anrufen</a>{status !== "ZUGESTELLT" ? <Button onClick={() => updateStoppStatus.mutate({ routeId: route.id, stoppId: stopp.id, status: "ZUGESTELLT" })}><PackageCheck size={16} aria-hidden /> Erfolgreich zugestellt</Button> : <div className="flex items-center justify-center gap-2 rounded-lg bg-ok-soft px-4 py-3 text-sm font-semibold text-ok"><CheckCircle2 size={17} aria-hidden /> Zugestellt</div>}{status !== "PROBLEM" && status !== "ZUGESTELLT" ? <Button variant="danger" onClick={() => updateStoppStatus.mutate({ routeId: route.id, stoppId: stopp.id, status: "PROBLEM", problemHinweis: "Vom Fahrer gemeldet" })}><AlertTriangle size={16} aria-hidden /> Problem melden</Button> : null}{status === "PROBLEM" ? <Button variant="secondary" onClick={() => updateStoppStatus.mutate({ routeId: route.id, stoppId: stopp.id, status: "OFFEN" })}><RouteIcon size={16} aria-hidden /> Problem geklärt</Button> : null}</div>
            </div>
          </Card>;
        })}
      </div>
