...
import { duplicatePlan, updatePlanStatus, useSpeiseplaene } from "../store";
        ...
        <tr key={p.id} className="hover:bg-paper">
          ...
          <Td>
            <div className="flex gap-3 text-xs font-medium no-print">
              {p.status === "REVIEW" && <button type="button" onClick={() => updatePlanStatus(p.id, "PUBLISHED")} className="cursor-pointer text-basil hover:underline">Veröffentlichen</button>}
              {p.status === "DRAFT" && <button type="button" onClick={() => updatePlanStatus(p.id, "REVIEW")} className="cursor-pointer text-basil hover:underline">Zur Prüfung</button>}
              <button type="button" onClick={() => duplicatePlan(p.id)} className="cursor-pointer text-muted hover:text-ink hover:underline">Duplizieren</button>
            </div>
          </Td>
        </tr>
      ))}
